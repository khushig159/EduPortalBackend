...
exports.signupRecruiter = async (req, res, next) => {
    try {
        ...
        const existingUser = await Mentor.findOne({ email: req.body.email });
        if (existingUser) {
        ...
        const { name, email, password, Exp, Organisation, Qualification,personality } = req.body;
        ...
        const savedUser = await mentor.save();
        ...
        setTimeout(async () => {
            try {
                const staleUser = await Mentor.findById(savedUser._id).lean();
        ...
        const createdUser = await Mentor.findById(savedUser._id).select(
...
exports.verifyEmailRecruiter = async (req, res, next) => {
    ...
    try {
        ...
        const user = await Mentor.findById(decoded.userId);
        if (!user) {
...
exports.loginRecruiter = async (req, res, next) => {
    ...
    try {
        ...
        const recruiter = await Mentor.findOne({ email });
        if (!recruiter) {
        ...
        const loggedInUser = await Mentor.findById(recruiter._id).select("-password -refreshTokenrecruiter");
